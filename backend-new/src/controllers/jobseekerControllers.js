...
const jobseekerGetJobDetail = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(`Có lỗi khi lấy thông tin công việc: ${err.message}`, 500)
    );
  }
};
...
const updateJobseekerProfileImage = async (req, res, next) => {
  try {
    ...
    try {
      ...
      if (affectedRows === 0) {
        return next(
          new ApiError("Cập nhật ảnh đại diện không thành công", 400)
        );
      }
      return res.success({}, "Cập nhật ảnh đại diện thành công");
    } catch (uploadErr) {
      return next(
        new ApiError(`Lỗi khi tải lên ảnh: ${uploadErr.message}`, 500)
      );
    }
  } catch (err) {
...
const getItemProfile = async (req, res, next) => {
  try {
    ...
    if (!type || !profile_id) {
      return next(
        new ApiError("Thiếu thông tin loại hồ sơ hoặc ID hồ sơ", 400)
      );
    }
    ...
    return next(
      new ApiError(`Có lỗi khi lấy thông tin hồ sơ: ${err.message}`, 500)
    );
  }
};
...
const deleteItemProfile = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(`Lỗi khi xóa thông tin hồ sơ: ${err.message}`, 500)
    );
  }
};
...
const addItemProfile = async (req, res, next) => {
  try {
    ...
    return next(
      new ApiError(`Lỗi khi thêm thông tin hồ sơ: ${err.message}`, 500)
    );
  }
};
...
const addResume = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return res.success({ resumes }, "Resume uploaded successfully");
    } catch (uploadErr) {
      return next(
        new ApiError(`Error uploading resume: ${uploadErr.message}`, 500)
      );
    }
  } catch (err) {
...
const deleteResume = async (req, res, next) => {
  try {
    ...
    return res.success({ resumes }, "Resume deleted successfully");
  } catch (err) {
...
const getListJobApplication = async (req, res, next) => {
  try {
    ...
    if (!data) {
      return next(
        new ApiError("Không tìm thấy danh sách công việc đã ứng tuyển", 404)
      );
    }
    return res.success(
      { jobs: data || [] },
...
const applyToJob = async (req, res, next) => {
  try {
    ...
    if (!job_id || !profile_id) {
      return next(
        new ApiError("Thiếu thông tin ID bài đăng hoặc ID người dùng", 400)
      );
    }
...
const addCompanyReview = async (req, res, next) => {
  try {
    ...
    if (!company_id || !profile_id || !score) {
      return next(
        new ApiError("Thiếu thông tin ID công ty hoặc ID người dùng", 400)
      );
    }
    const success = await queryAddCompanyReview(
      profile_id,
      company_id,
      score,
      content
...
const getListCompanyFollowing = async (req, res, next) => {
  try {
    ...
    if (!data) {
      return next(
        new ApiError("Không tìm thấy thông tin công ty đã theo dõi", 404)
      );
    }
    return res.success(data, "Lấy danh sách công ty đã theo dõi thành công");
  } catch (err) {
...
const getListJobSaving = async (req, res, next) => {
  try {
    ...
    if (!data) {
      return next(
        new ApiError("Không tìm thấy thông tin bài đăng đã lưu", 404)
      );
    }
    return res.success(
      { jobs: data || [] },
...
const getJobsSuggestion = async (req, res, next) => {
  try {
    ...
    if (!data) {
      return next(
        new ApiError("Không tìm thấy thông tin công việc gợi ý", 404)
      );
    }
    return res.success(
      { jobs: data || [] },
...
const getNotification = async (req, res, next) => {
  try {
    ...
    return res.success(data || [], "Lấy danh sách thông báo thành công");
  } catch (err) {
...
const updateReadNotification = async (req, res, next) => {
  try {
    ...
    const result = await queryUpdateReadNotification(
      profile_id,
      notification_id
    );
