...
FROM 
    job j
LEFT JOIN 
    employer e ON j.employer_id = e.employer_id
LEFT JOIN 
...
LEFT JOIN 
    catalog_education edu ON j.education_at_least = edu.education_id
ORDER BY 
    j.date_post DESC
LIMIT 30;
    `);
...
const queryGetWorkDetail = async (workId) => {
  const [work] = await db.query(
    ...
    (SELECT GROUP_CONCAT(js.skill SEPARATOR ', ')
...
LEFT JOIN
    catalog_education edu ON j.education_at_least = edu.education_id
LEFT JOIN
    company_benifit cb ON c.company_id = cb.company_id
LEFT JOIN
...
const queryGetWorkBySearch = async (filter) => {
  ...
  if (education_at_least) {
    conditions.push("education_at_least >= ?");
    values.push(education_at_least);
  }
